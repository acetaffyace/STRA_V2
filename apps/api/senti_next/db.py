...
def init_db() -> None:
    ...
    # Stage 3B discovery is an independent, read-only audit of one immutable
    # semantic index.  It never adds fields to Research Core or taxonomy rows.
    if database in (None, ":memory:"):
        with get_connection() as conn:
            raw = conn.connection.driver_connection
            if migrations.current_version(raw) < semantic_discovery_schema.SEMANTIC_DISCOVERY_MIGRATION_VERSION:
                semantic_discovery_schema.migrate_semantic_discovery(raw)
                migrations.record_version(
                    raw,
                    semantic_discovery_schema.SEMANTIC_DISCOVERY_MIGRATION_VERSION,
                    semantic_discovery_schema.DESCRIPTION,
                )
                raw.commit()
    else:
        discovery_path = Path(database).expanduser().resolve()
        discovery_backup = discovery_path.with_name(discovery_path.name + ".semantic_discovery_v1.bak")
        migrations.apply_ordered_migrations(
            discovery_path,
            [(
                semantic_discovery_schema.SEMANTIC_DISCOVERY_MIGRATION_VERSION,
                semantic_discovery_schema.DESCRIPTION,
                semantic_discovery_schema.migrate_semantic_discovery,
            )],
            backup_path=discovery_backup,
            restore_on_error=True,
        )

    # Stage 3B.1 keeps the v15 discovery structure history intact and adds an
    # independent context-materialization cache.  The migration is additive
    # and safe to run repeatedly on both fresh and existing databases.
    if database in (None, ":memory:"):
        with get_connection() as conn:
            raw = conn.connection.driver_connection
            if migrations.current_version(raw) < semantic_discovery_materialization_schema.SEMANTIC_DISCOVERY_MATERIALIZATION_MIGRATION_VERSION:
                semantic_discovery_materialization_schema.migrate_semantic_discovery_materializations(raw)
                migrations.record_version(
                    raw,
                    semantic_discovery_materialization_schema.SEMANTIC_DISCOVERY_MATERIALIZATION_MIGRATION_VERSION,
                    semantic_discovery_materialization_schema.DESCRIPTION,
                )
                raw.commit()
    else:
        materialization_path = Path(database).expanduser().resolve()
        materialization_backup = materialization_path.with_name(materialization_path.name + ".semantic_discovery_v2.bak")
        migrations.apply_ordered_migrations(
    ...
    # M2 persists SemanticRun configuration identity separately from its
    # execution/result payload so configuration changes create new runs.
    if database in (None, ":memory:"):
        with get_connection() as conn:
            raw = conn.connection.driver_connection
            if migrations.current_version(raw) < semantic_run_schema.SEMANTIC_RUN_MIGRATION_VERSION:
                semantic_run_schema.migrate_semantic_runs(raw)
                migrations.record_version(raw, semantic_run_schema.SEMANTIC_RUN_MIGRATION_VERSION, semantic_run_schema.DESCRIPTION)
                raw.commit()
            else:
                semantic_run_schema.migrate_semantic_runs(raw)
                raw.commit()
    else:
        semantic_run_path = Path(database).expanduser().resolve()
        semantic_run_backup = semantic_run_path.with_name(semantic_run_path.name + ".semantic_run_v1.bak")
        migrations.apply_ordered_migrations(
            semantic_run_path,
            [(semantic_run_schema.SEMANTIC_RUN_MIGRATION_VERSION, semantic_run_schema.DESCRIPTION, semantic_run_schema.migrate_semantic_runs)],
            backup_path=semantic_run_backup,
            restore_on_error=True,
        )

    # M2 evidence identity stores UTF-8 byte-addressed semantic units and
    # cardinality-safe topic mentions separately from semantic-run state.
    if database in (None, ":memory:"):
        with get_connection() as conn:
            raw = conn.connection.driver_connection
            if migrations.current_version(raw) < semantic_unit_schema.SEMANTIC_UNIT_MIGRATION_VERSION:
                semantic_unit_schema.migrate_semantic_units(raw)
                migrations.record_version(raw, semantic_unit_schema.SEMANTIC_UNIT_MIGRATION_VERSION, semantic_unit_schema.DESCRIPTION)
                raw.commit()
            else:
                semantic_unit_schema.migrate_semantic_units(raw)
                raw.commit()
    else:
        semantic_unit_path = Path(database).expanduser().resolve()
        semantic_unit_backup = semantic_unit_path.with_name(semantic_unit_path.name + ".semantic_unit_v1.bak")
        migrations.apply_ordered_migrations(
            semantic_unit_path,
