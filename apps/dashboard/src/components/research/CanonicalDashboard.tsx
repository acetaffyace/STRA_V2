...
function MetricList({ title, group, onSelect }: { title: string; group: DashboardMetricGroup; onSelect: (row: DashboardMetricRow) => void }) {
  return (
    <section className="rounded-xl border border-white/10 bg-slate-950/35 p-4" aria-labelledby={`metric-${title}`}>
      ...
      {group.items.length === 0 ? <p className="text-sm text-slate-500">Unavailable for this run.</p> : (
        <div className="space-y-3">
          {group.items.map((row) => (
            <button key={row.taxonomy_key} type="button" onClick={() => onSelect(row)} className="group block w-full text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300">
              ...
              <div className="mt-1 flex justify-between text-[10px] text-slate-500"><span>{percent(row.share)} of classified reviews</span><span>View source reviews</span></div>
            </button>
          ))}
        </div>
      )}
...
export function CanonicalDashboard({ presentation, appName, appId }: { presentation: DashboardPresentation; appName: string; appId: number }) {
  ...
  async function openEvidence(row: DashboardMetricRow) {
    setSelected(row); setEvidence(null); setEvidenceError(null);
    try { setEvidence(await fetchAnalysisEvidence(appId, row.taxonomy_key, runId, 5)); }
    catch { setEvidenceError("Source reviews are temporarily unavailable."); }
  }

  return (
    <main className="mx-auto max-w-7xl space-y-5 px-4 pb-10" data-testid="canonical-dashboard">
      <section className="rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900/95 to-slate-950/90 p-5 shadow-xl shadow-black/10">
      ...
      <section><div className="mb-3 flex items-end justify-between"><div><p className="text-xs uppercase tracking-widest text-cyan-300/70">Player voice</p><h2 className="mt-1 text-xl font-semibold text-white">Actionable topics</h2></div><p className="text-xs text-slate-500">Observed classified review share</p></div><MetricList title="Actionable Topics" group={presentation.player_voice.actionable_topics} onSelect={openEvidence} /></section>
      <section className="grid gap-4 lg:grid-cols-2"><MetricList title="Issues" group={presentation.player_voice.issues} onSelect={openEvidence} /><MetricList title="Requests" group={presentation.player_voice.requests} onSelect={openEvidence} /></section>
