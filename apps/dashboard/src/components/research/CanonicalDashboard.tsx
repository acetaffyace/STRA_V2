  ...
  return (
    <section className="rounded-lg border border-white/10 bg-slate-950/25 p-4" aria-labelledby={`metric-${title}`}>
      <div className="mb-3 flex items-baseline justify-between gap-3">
        ...
        <span className="text-[11px] text-slate-500">{group.total_count} 项</span>
      </div>
...
export function CanonicalDashboard({ presentation, appName, appId, showHeader = true, onConfigureSampling }: { presentation: DashboardPresentation; appName: string; appId: number; showHeader?: boolean; onConfigureSampling?: () => void }) {
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-5 px-4 pb-8" data-testid="canonical-dashboard">
      {!showHeader && <section className="rounded-lg border border-white/10 bg-slate-900/60 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs uppercase tracking-widest text-slate-500">评论概览</p><h1 className="mt-1 text-xl font-semibold text-white" title={appName}>{appName}</h1><p className="mt-1 text-xs text-slate-400">{scopeOrder} · {scopeLanguages}</p></div><div className="flex flex-wrap items-end gap-5"><div><p className="text-xs text-slate-500">好评率</p><p className="text-2xl font-semibold text-white">{percent(snapshot.recommendation_rate)}</p></div><div><p className="text-xs text-slate-500">评论</p><p className="text-lg font-medium text-white">{snapshot.population_n}</p><p className="text-xs text-slate-400">{snapshot.population_n ? `${snapshot.valid_n} 条有效评论` : "暂无评论"}</p></div><div><p className="text-xs text-slate-500">分析状态</p><p className="text-lg font-medium text-white">{semantic.available ? `${semantic.classified_n ?? 0} / ${semantic.population_n ?? snapshot.population_n}` : "语义分析不可用"}</p><p className="text-xs text-amber-200">{semantic.available ? "暂定结果 ⓘ" : "仅显示评论概览"}</p></div>{onConfigureSampling && <button type="button" onClick={onConfigureSampling} className="rounded border border-sky-400/30 px-3 py-2 text-xs text-sky-200 hover:bg-sky-400/10">更新分析</button>}</div></div></section>}
      ...
      <section className="rounded-lg border border-white/10 bg-slate-900/60 p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-widest text-slate-500">分析状态</p><h2 className="mt-1 text-lg font-semibold text-white">语义分析</h2></div><span title="分析已完成，但语义分类模型尚未完成正式验证。" className={`rounded-md border px-3 py-1 text-xs font-bold ${semantic.available ? "border-amber-300/50 bg-amber-300/15 text-amber-100" : "border-slate-500/40 bg-slate-500/10 text-slate-300"}`}>{semantic.available ? "暂定结果 ⓘ" : "暂不可用"}</span></div>{semantic.available ? <><p className="mt-3 text-sm text-slate-300">语义分析已完成，但当前结果仍处于验证阶段。</p><div className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-3"><div><span className="text-slate-500">已分析评论</span><strong className="mt-1 block text-white">{semantic.classified_n ?? 0} / {semantic.population_n ?? snapshot.population_n}</strong></div><div><span className="text-slate-500">分析覆盖率</span><strong className="mt-1 block text-white">{percent(semantic.classification_coverage)}</strong></div><div><span className="text-slate-500">结果资格</span><strong className="mt-1 block text-amber-100">暂定</strong></div></div></> : <p className="mt-3 text-sm text-slate-400">语义分析不可用：{semantic.reason || "这次分析没有语义结果"}。评论概览仍然可用。</p>}</section>
