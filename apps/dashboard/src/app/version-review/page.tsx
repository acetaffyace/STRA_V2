...
function VersionReviewContent() {
  ...
  useEffect(() => {
    ...
    if (embedded.window_days === selectedWindow) { setV2Comparison(embedded); return; }
    fetchVersionComparison(run.run_id, selectedWindow).then((response) => setV2Comparison(response.comparison)).catch(() => setV2Comparison(null));
  }, [run, selectedWindow]);
