...
def test_stage3d_migration_18_is_idempotent_and_baseline_is_materialized(monkeypatch):
    ...
    try:
        with db.get_connection() as conn:
            ...
            assert conn.execute(text("SELECT MAX(version) FROM schema_migrations")).scalar() == 18
            assert migrations.schema_status(raw) == {"latest_known": 18, "applied": 18, "status": "current"}
    finally:
        db.close_engine()
        db._engine = None
